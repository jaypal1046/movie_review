...
class _HomeScreenState extends State<HomeScreen> {
    ...
    for (var i = 0; i < labels.length; i++) {
      ...
      Widget child = Container(
        ...
        child: ElevatedButton(
          ...
          child: Padding(
            padding: _padding.add(const EdgeInsets.symmetric(horizontal: 12)),
            child: Text(
              labels[i],
              style: _textStyle,
            ),
  ...
  void initState() {
    ...
    _provider = Provider.of<ShowsProvider>(context, listen: false);
  ...
  Widget build(BuildContext context) {
    ...
    final Widget home = RefreshIndicator(
        onRefresh: ()async{
          _provider?.refresh();
